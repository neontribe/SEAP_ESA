...
if [ "$TRAVIS_TAG" ]; then
    ...
    sshpass -p $DEPLOY_PASS ssh $DEPLOY_USER@$DEPLOY_HOST  "cd $DEPLOY_PATH && tar -xvf release.tgz && mv seap_esa SEAP_ESA && rm -rf SEAP_ESA/.git && printf "$TRAVIS_TAG-$DATETIME" > SEAP_ESA/release.txt && rm release.tgz && bash deploy-seap-esa.sh"
